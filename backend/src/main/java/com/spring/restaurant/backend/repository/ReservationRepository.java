...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query(value = "SELECT DISTINCT r.* " +
        "FROM reservation r " +
        "JOIN contains_tables ct ON r.id = ct.reservation_id " +
        "JOIN restaurant_table t ON ct.restaurant_table_id = t.id " +
        "WHERE (:guestName IS NULL OR UPPER(r.guest_name) LIKE CONCAT('%', UPPER(:guestName), '%')) " +
        "AND (:tableNum IS NULL OR t.table_num = :tableNum) " +
        "AND ((:startDateTime IS NULL OR r.end_date_time > :startDateTime) " +
        "AND (:endDateTime IS NULL OR r.start_date_time < :endDateTime)) " +
