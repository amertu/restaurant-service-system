...
public class TableAndReservationDataGenerator {
    ...
    private void generateReservations() {
        LOGGER.info("Starting generateReservations");
        if (!reservationRepository.findAll().isEmpty()) {
            ...
            for (int i = 0; i < NUMBER_OF_RESERVATIONS_TO_GENERATE; i++) {
                ...
                tables.removeIf(table -> !table.getActive());
                if (tables.isEmpty()) {
                    for (RestaurantTable table : createdTables) {
                        if (table.getActive()) {
                            tables.add(table);
                            break;
                        }
                ...
                int numberOfGuests = Util.calculateTotalNumberOfSeats(tables);
        ...
        LOGGER.info("Finished generateReservations");
    }
